...
router.post('/api/get-in-touch', (req, res) => {
  ...
  getInTouchService.saveGetInTouch({ name, email, phone, subject, message })
    .then(() => {
      res.status(200).json({
        success: true,
...
//FAQ API Route
router.post('/api/faq-submit', (req, res) => {
  const { name, email, phone, subject, message } = req.body;

  console.log('FAQ:', { name, email, phone, subject, message });
  // Future: save to DB / send email
  getInTouchService.saveFAQ({ name, email, phone, subject, message })
    .then(() => {
      res.status(200).json({
        success: true,
        message: 'Thanks for reaching out! We will get back to you shortly.'
    ...
    .catch(error => {
      ...
      res.status(500).json({
        success: false,
        message: 'An error occurred while saving your FAQ information.'
      });
    });
});
